...
def look_up_table(max_speed, min_speed, max_torque, min_torque, speed_dif,
    ...
    is_speed_torque_vd = \
        (data_frame['Speed(RPM)'].between(min_speed, max_speed)) & \
        (data_frame['Torque(Nm)'].between(min_torque, max_torque))
    ...
    if(min_speed == 4000 and min_torque == 180):
        ...
        in_speed_torque_fc = (fcf['Speed'] == (
            max_speed - speed_dif)) & (fcf['Torque'] == 15)

    # Last element in speed array
    elif(min_speed == 4000):
        in_speed_torque_fc = (fcf['Speed'] == 4000) & (
            fcf['Torque'] == (max_torque - torque_diff))

    # Last element in torque array
    elif(min_torque == 180):
        in_speed_torque_fc = (fcf['Speed'] == (
            max_speed - speed_dif)) & (fcf['Torque'] == 180)

    elif(max_speed == 0):
        in_speed_torque_fc = (fcf['Speed'] == 0) & (
            fcf['Torque'] == (max_torque - torque_diff))

    else:
        in_speed_torque_fc = \
            (fcf['Speed'] == (max_speed - speed_dif)) & \
            (fcf['Torque'] == (max_torque - torque_diff))

    # Filter [speed][torque] in vehicle data
    filt_in_speed_torque_vd = data_frame[is_speed_torque_vd]
    ...
    if ((not filt_in_speed_torque_fc.empty)
       and (not filt_in_speed_torque_vd.empty)):
        ...
        for index in filt_in_speed_torque_vd.index:
            data_frame.loc[index, 'FuelConsumption'] = look_up_fuel_consumption


def main(data_frame_file_name, look_up_table_file_name):
    '''
    Performs addition of the given input variables
    @type  data_frame_file_name: string
    @param data_frame_file_name: File path of data frame
    @type  look_up_table_file_name: string
    @param look_up_table_file_name: File path of look-up table

    Iterating through rows in fuel consumption look-up table and comparing
    look-up table values with data_frame values. If the same, then fuel
    consumption value is placed in the appropriate index

    For debugging use the file names below
    data_frame = pd.read_csv('vehicle_data.csv')
    Fuel consumption file
    fcf = pd.read_csv('fuel_consumption.csv')
    '''

    data_frame = pd.read_csv(data_frame_file_name)
    # #Fuel consumption file
    fcf = pd.read_csv(look_up_table_file_name)

    # Converting 2-D Look-up table to a two column and result format
    fcf = pd.melt(fcf, id_vars=['Speed'], value_vars=[
                  '15.0', '30.0', '45.0', '60.0', '75.0', '90.0', '105.0',
                  '120.0', '135.0', '150.0', '165.0', '180.0'
                  ])
    ...
    max_speed_in_data_frame = data_frame['Speed(RPM)'].max()
    min_speed_in_data_frame = data_frame['Speed(RPM)'].min()
    max_torque_in_data_frame = data_frame['Torque(Nm)'].max()
    min_torque_in_data_frame = data_frame['Torque(Nm)'].min()

    # Creating new column with all zeroes
    data_frame['FuelConsumption'] = 0.0
    # Iterating through look-up table
    for index, row in fcf.iterrows():

        # Iterating through the min speed and torque and the diff between
        # each speed and each torque
        for min_speed, min_torque in zip(unique_speeds, unique_torque):
            ...
            if(row['Speed'] == 4000 and row['Torque'] == 180):
                look_up_table(
                    max_speed_in_data_frame, row['Speed'],
                    max_torque_in_data_frame, 180,
                    speed_dif, torque_diff, data_frame, fcf
                    )

            elif(row['Speed'] == 4000 and row['Torque'] == 15):
                look_up_table(max_speed_in_data_frame, 4000, 15,
                              min_torque_in_data_frame, speed_dif,
                              torque_diff, data_frame, fcf)

            # Lowest speed and lowest torque
            elif(row['Speed'] == 0 and row['Torque'] == 15):
                max_torque = torque_diff + row['Torque']
                look_up_table(0, min_speed_in_data_frame, 15,
                              min_torque_in_data_frame, speed_dif,
                              torque_diff, data_frame, fcf)
            ...
            # Last element in speed array
                ...
                max_torque = torque_diff + row['Torque']
                look_up_table(max_speed_in_data_frame, 4000, max_torque,
                              row['Torque'], speed_dif, torque_diff,
                              data_frame, fcf)
            ...
            # Last element in torque array
                ...
                look_up_table(
                    max_speed, row['Speed'], max_torque_in_data_frame, 180,
                    speed_dif, torque_diff, data_frame, fcf)
            ...
            # Lowest torque
                ...
                max_speed = speed_dif + row['Speed']
                look_up_table(
                    max_speed, row['Speed'], 15, min_torque_in_data_frame,
                    speed_dif, torque_diff, data_frame, fcf)

            # Lowest speed
            elif row['Speed'] == 0:
                max_torque = torque_diff + row['Torque']
                look_up_table(0, min_speed_in_data_frame, max_torque,
                              row['Torque'], speed_dif, torque_diff,
                              data_frame, fcf)
...
if len(sys.argv) > 2:
    ...
    print()
    print('Please re-run script and specify name of')
    print('data_frame file and look-up table without extension')
    sys.exit()
